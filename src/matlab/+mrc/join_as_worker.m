...
try
    ...
    if strcmp(db_id, get_db_id())
        ...
        for idx = 1:numel(dependent_tasks)
            ...
            lua_update_dependent_task = char(strcat("""", ...
                ...
                "if redis.call('LLEN', dependent_task_key .. ':dependencies') == 0 then ", ...
                    "if redis.call('LREM', 'pre_pending_tasks', 0, dependent_task_key) == 0 then return -1 end; ", ...
                    "redis.call('LPUSH', 'pending_tasks', dependent_task_key); ", ...  
                    "redis.call('HSET', dependent_task_key, 'status', 'pending'); ", ...
                "end;", ...
                "return 0;", ...
                """ 2"));
            mrc.redis_cmd(['EVAL ' lua_update_dependent_task ' ' dependent_task_key ' ' task_key]);
        end
        mrc.redis_cmd({'MULTI', ...
