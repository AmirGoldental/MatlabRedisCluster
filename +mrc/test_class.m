classdef test_class < matlab.unittest.TestCase
    ...
    methods(Test)        
        ...
        function test_get_redis_hash(testCase)  
            ...
            mrc.redis_cmd('hmset test_hash field1 value1 field2 "value 2" field3_numeric 0');
        ...
        function functional_DAG_test_2(testCase)
            t1 = mrc.new_task(repmat({'mrc.redis_cmd(''RPUSH test_list task0'')'},3,1));
            mrc.new_task('fail');
            expected_test_list = repmat({'task0'},3,1);
            t2 = mrc.new_task(repmat({'mrc.redis_cmd(''RPUSH test_list task1'')'},3,1), 'dependencies', t1);
            expected_test_list = [expected_test_list; repmat({'task1'},3,1)];
            mrc.new_task('fail');
            mrc.new_task(repmat({'mrc.redis_cmd(''RPUSH test_list task2'')'},3,1), 'dependencies', [t1;t2]);
